...
# Página principal com filtros, contadores e painel de detalhes
@app.route('/')
def view_tasks():
    ...
    # Título da página conforme filtro
    ...
    if sort_by == 'due_date':
        ...
        query = query.order_by(priority_order if order == 'asc' else priority_order.desc())
    else:
        query = query.order_by(Task.created_at.asc() if order == 'asc' else Task.created_at.desc())

    tasks = query.all()
    selected_task = Task.query.get(selected_id) if selected_id else None
    all_lists = TaskList.query.all()
    selected_list = TaskList.query.get(list_id) if list_id else None

    # Contadores para filtros e listas
    counts = {
...
# Excluir lista (mantém tarefas, apenas remove referência)
...
# Rota para exibir todas as tarefas (sem filtro)
@app.route('/all_tasks')
def index():
    ...
    all_lists = TaskList.query.all()
    return render_template('index.html', tasks=tasks, counts=counts, all_lists=all_lists)
